...
The 2026-06-13 jungle vegetation pass starts the runtime ownership split:
near-field ground cover is generated by `JungleGroundRing` through
`GlobalBillboardSystem`, while `VegetationScatterer` keeps non-ground-cover
mid/far vegetation. `fanPalm` and `coconut` are the first accepted canopy/tree
tier families for runtime placement, backed by their existing Pixel Forge
impostor assets. This does not approve any blocked vegetation species or
promote generic Pixel Forge prop trees into runtime vegetation.
