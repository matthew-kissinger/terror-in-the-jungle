...
The proof-fixture `createAlphaTextureNodeMaterial` in `src/core/TslMaterialFactory.ts:83` and the `KonveyerInstancedSlice` wrapper (removed in cycle-2026-06-09 delete-orphan-modules) are **not** wired into production scenes; their only non-test caller was the konveyer-slice probe script (removed with the spike). They are excluded from the production ranking above.
