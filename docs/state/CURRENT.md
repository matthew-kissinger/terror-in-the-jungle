...
- **2026-06-17 local / 2026-06-18 UTC — active-driver route recovery checkpoint**:
  ...
  nearest-OPFOR `combat_approach_unavailable` path fails, instead of only
  attempting to cool down `currentTarget` and immediately reacquiring the same
  unreachable combatant. Missing current-target telemetry now reports unknown
  liveness rather than `live=true`, reducing false confidence in PATROL/failed
  route windows. Focused proof is `node --check scripts/perf-active-driver.cjs`,
