...
- **2026-06-17 local / 2026-06-18 UTC — active-driver route recovery checkpoint**:
  ...
  nearest-OPFOR route stops making progress. Because route overlay passes
  position-only anchors, the cooldown helper now derives `nearest_opfor` ids
  from route identity keys such as `nearest_opfor:combatant_52`; otherwise the
  blocked-target map never skipped that combatant on the next objective search.
  Missing current-target telemetry reports unknown liveness rather than
  `live=true`, reducing false confidence in PATROL/failed route windows.
  Focused proof is `node --check scripts/perf-active-driver.cjs`,
  `npx vitest run scripts/perf-harness/perf-active-driver.test.js`,
  `npx vitest run scripts/check-dropped-frame-ears.test.ts`, and targeted
  ESLint. This improves the next evidence pass but is not dropped-frame
  completion evidence.
