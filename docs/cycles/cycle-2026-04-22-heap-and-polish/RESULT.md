...
Cycle-specific acceptance results:
  ...
  - 2026-04-24 recovery caveat: current docs supersede the broad "all five modes" cloud sign-off above. `npm run evidence:atmosphere` now validates all five modes through sky-dome clouds after preview builds began emitting `asset-manifest.json`. The old `CloudLayer` plane is hidden, and A Shau no longer uses the removed TileCache fallback path; static-tiled route/NPC quality still needs a real nav gate. See `docs/ATMOSPHERE.md` and `docs/STATE_OF_REPO.md` before using this historical result as current truth.
