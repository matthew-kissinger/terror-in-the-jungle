...
| combat-movement-stall-tail | Combat120 p99 movement-stall tail | open | combat / navigation | split from DEFEKT-3 2026-06-03 (cover search proven cheap via `CoverSearch.bench.test.ts` + tail attribution); spike [COMBAT_AI_P99_SPIKE](rearch/COMBAT_AI_P99_SPIKE_2026-06-03.md); attribution in `scripts/perf-tail-attribution.ts`; `crowdStallStaggerEnabled` now default ON | Reduce/eliminate the NPC terrain-stall oscillation in `CombatantMovement` (the residual Combat-phase p99 tail); combat120 p99 ≤35ms once the render/Other superposition + STABILIZAT-1 quiet-box capture are resolved |
