...
| DEFEKT-3 | Combat AI p99 anchor | open (active; O(1) path wired; perf unproven) | combat | `cover-grid-wiring` (cycle-2026-05-28) wired O(1) `CoverSpatialGrid` into prod combat; combat120 p99 PASS unproven — no baseline is tracked (`perf-baselines.json` removed) so `perf:compare` is non-gating; see [perf-trust.md](state/perf-trust.md); spike [COMBAT_AI_P99_SPIKE](rearch/COMBAT_AI_P99_SPIKE_2026-06-03.md) (cover search ≈0 — real tail is movement stall); cycle `combat-p99-attribution` | Sync cover search in `AIStateEngage.initiateSquadSuppression` no longer dominates p99; combat120 p99 ≤35ms PASS |
