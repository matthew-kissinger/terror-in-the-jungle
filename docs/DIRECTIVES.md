...
Last verified: 2026-06-03 (cycle-open: DIZAYN-4 Field Journal frontend → code-complete / playtest-deferred; opened a big cycle of 3 parallel streams — `deploy-loadout-flow` / `air-support-radio` / `combat-p99-attribution` — each with a spike memo under [`docs/rearch/`](rearch/). Slim refactor under `directives-slim-refactor`; full evidence prose lives in per-id memo files under [`docs/directives/`](directives/)).
...
| SVYAZ-3 | Air-support call-in radio | open (active) | combat / UI / aviation | first slice `665b0c5` (radio shell + asset list); spike [AIR_SUPPORT_RADIO_SPIKE](rearch/AIR_SUPPORT_RADIO_SPIKE_2026-06-03.md); cycle `air-support-radio` | Radio menu + target marking + asset selection + per-asset cooldown + NPC-pilot fulfillment |
...
| UX-2 | Map spawn / respawn flow | open (active) | UI / player | spike [DEPLOY_LOADOUT_FLOW_SPIKE](rearch/DEPLOY_LOADOUT_FLOW_SPIKE_2026-06-03.md); cycle `deploy-loadout-flow` | Map shows spawn options; tap- and click-to-spawn; mobile touch targets sized correctly |
| UX-3 | Loadout selection | open (active) | UI / player | spike [DEPLOY_LOADOUT_FLOW_SPIKE](rearch/DEPLOY_LOADOUT_FLOW_SPIKE_2026-06-03.md); cycle `deploy-loadout-flow` (selectable ammo — fence-gated on `IAmmoManager`) | Loadout categories + ammo loads + faction availability + PC/mobile parity |
| UX-4 | Deploy flow polish | open (active) | UI / player | spike [DEPLOY_LOADOUT_FLOW_SPIKE](rearch/DEPLOY_LOADOUT_FLOW_SPIKE_2026-06-03.md); cycle `deploy-loadout-flow` | Menu-to-first-frame is fast + clear; immediate danger readable in first frame |
