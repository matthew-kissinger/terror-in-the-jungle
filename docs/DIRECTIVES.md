...
| SVYAZ-4 | RTS-flavored command discipline | open (Stage 1 of 4 landed) | combat / UI | spike [SQUAD_COMMAND_SVYAZ4_SPIKE](rearch/SQUAD_COMMAND_SVYAZ4_SPIKE_2026-06-03.md) (root cause: ground orders are only read in `PATROLLING` and die on enemy contact; hotkey targeting defaulted to the player's own feet). **Stage 1 (targeting truth) landed**: look-to-mark (camera→ground pick, reused from air-support) + map-click now ping a real world point, never the player's feet; ATTACK added on Shift+6; L2-tested. Stages 2-4 pending — combat persistence via an order-leash, per-order posture (ATTACK-advance / FALL-BACK rally), robustness. | Squad + air-support commands compose so the sim reads as hybrid FPS/RTS; ground orders survive combat with disciplined leash behavior |
...
| combat-movement-stall-tail | Combat120 p99 movement-stall tail | open (F2 landed default-OFF) | combat / navigation | split from DEFEKT-3 2026-06-03; spike [COMBAT_AI_P99_SPIKE](rearch/COMBAT_AI_P99_SPIKE_2026-06-03.md); attribution in `scripts/perf-tail-attribution.ts`. **F2 sticky contour hysteresis landed default-OFF** (`NpcLodConfig.contourStickyHysteresisEnabled`; `scoreContourDirection` same-side bonus 0.25→1.0 gated): an L1 getHeightAt measurement REFUTED the dedup hypothesis (per-tick terrain sampling only ~12% redundant), so the lever is reducing stall-tick COUNT, not re-sampling; A/B cuts contour side-flips (11→0 on an asymmetric ridge) and the NPC clears the lip; combat-reviewer APPROVE-WITH-NOTES; playtest + flip-on gate in [PLAYTEST_PENDING](PLAYTEST_PENDING.md). `crowdStallStaggerEnabled` default ON. | Reduce/eliminate the NPC terrain-stall oscillation in `CombatantMovement` (the residual Combat-phase p99 tail); combat120 p99 ≤35ms once the render/Other superposition + STABILIZAT-1 quiet-box capture are resolved |
