...
1. Triage the remaining `combat120` tail spikes in high-LOD `suppressing` / `advancing` updates now that AI query churn is reduced. Friendly-spacing removal on visual-only high-LOD frames was not enough to keep.
...
6. Hold WebGPU, WASM, worker offload, and navmesh adoption until the low-friction CPU fixes above are re-measured against warm baselines.
