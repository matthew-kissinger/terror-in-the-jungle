...
- Keep: World boundary enforcement now reads playable bounds explicitly from `ITerrainRuntime` (`getPlayableWorldSize()` with `getWorldSize()` fallback) in `PlayerMovement.updateMovement()` and `HelicopterModel.updateHelicopterPhysics()`. Boundary hit bounces velocity inward at 50% strength. This preserves init-order resilience while removing the semantic overload that treated one size as both gameplay and render overflow.
