...
*Status:* evidence-complete. The neutral-command source/test audit at
...
explicit squad stand-down order.
...
*Status:* evidence-complete. The refreshed source/test audit at
...
*Status:* evidence-in-progress, local visual packet signed. The refreshed
...
**DEFEKT-2 — Drift detection between docs and code.** *Status:* evidence-complete for the local doc/code/artifact drift gate. `check:doc-drift` runs `scripts/projekt-143-doc-drift.ts` against the codex, `docs/STATE_OF_REPO.md`, `docs/PERFORMANCE.md`, concrete `artifacts/perf/` references, and documented `npm run` command references.
...
In-flight (uncommitted): `vehicle-visuals-and-airfield-polish` + visual fallback / aircraft directionality audit (2026-05-08). Five gameplay polish fixes; aircraft import now preserves Huey / UH-1C Gunship `z` tail-rotor axes and applies an explicit AH-1 Cobra `x->z` runtime-axis correction. See STABILIZAT-2, AVIATSIYA-1, and DEFEKT-5.
