...
- **FlashbangScreenEffect** (`src/systems/player/FlashbangScreenEffect.ts`, 150 lines) - Full-screen white overlay with distance and angle-based intensity. 3s full blind at <15m, 1.5s partial at 15-25m. Player-only - NPCs are unaffected.
- **GrenadeEffects** (`src/systems/weapons/GrenadeEffects.ts`, 166 lines) - Dispatches frag/smoke/flashbang explosion logic.
...
- **GrenadeEffects.ts explodeFrag()/explodeSmoke() allocations** - Lines 62-68 and 99-105. Creates 15-30 Vector3 per detonation via `new Vector3()` and `.clone()`. Not per-frame but fixable with module-level scratch vectors.
...
- **77 `any` type annotations** across 34 files + 167 total including `as any` casts across 54 files (heaviest: SystemInterfaces.ts with 19 - intentional, WeaponFiring.ts with 18). Reduced from 135 via targeted refactoring.
...
~52.7k lines across 272 files. Orchestrator pattern with ongoing split refactors.
