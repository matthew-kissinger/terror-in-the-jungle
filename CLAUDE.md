...
- **73 `: any` type annotations** across ~30 files + 63 `as any` casts across ~21 files (heaviest: SystemInterfaces.ts with 19 `: any` - intentional). Reduced from 135 via targeted refactoring. WeaponFiring.ts `as any` casts eliminated. CombatantSystem.handlePlayerShot() return type fixed (commit ababb6f). IHUDSystem expanded to 28 methods (commit d449418). SystemConnector.ts has 9 `as any` casts accessing internal CombatantSystem subsystems.
...
~52.8k lines across 273 files. Orchestrator pattern with ongoing split refactors.
