...
- **Unit tests (Vitest)** - 39 test files, 1518 tests passing. Coverage: Logger, Math, NoiseGenerator, ObjectPoolManager, SpatialOctree, SpatialGrid, SpatialGridManager, HeightQueryCache, ChunkWorkerPool, InfluenceMapComputations, InfluenceMapGrid, InfluenceMapSystem, TerrainMeshMerger, ImprovedChunkManager, CombatantDamage, CombatantLODManager, CombatantRenderer, CombatantCombat, CombatantSpawnManager, TicketSystem, AICoverSystem, AIStatePatrol, AIStateDefend, AIStateEngage, AIFlankingSystem, GunplayCore, GrenadeSystem, MortarSystem, PlayerMovement, PlayerController, PlayerInput, PlayerSuppressionSystem, PlayerRespawnManager, WeaponFiring, FirstPersonWeapon, AmmoSupplySystem, InventoryManager, SmokeCloudSystem, WeaponPickupSystem. Run `npm run test:run` to execute.
