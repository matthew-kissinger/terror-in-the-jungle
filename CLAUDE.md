...
- **Unit tests (Vitest)** - 18 test files, 544 tests passing. Coverage: Logger, MathUtils, NoiseGenerator, ObjectPoolManager, SpatialOctree, SpatialGrid, HeightQueryCache, ChunkWorkerPool, InfluenceMapComputations, InfluenceMapGrid, TerrainMeshMerger, CombatantDamage, CombatantLODManager, TicketSystem, CombatantCombat, AICoverSystem, AIStatePatrol, AIStateDefend. Run `npm run test:run` to execute.
