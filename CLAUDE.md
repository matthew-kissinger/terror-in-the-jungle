...
- **Unit tests (Vitest)** - 15 test files, 487 tests passing. Coverage: Logger, MathUtils, NoiseGenerator, ObjectPoolManager, SpatialOctree, SpatialGrid, HeightQueryCache, ChunkWorkerPool, InfluenceMapComputations, InfluenceMapGrid, TerrainMeshMerger, CombatantDamage, CombatantLODManager, TicketSystem, CombatantCombat. Run `npm run test:run` to execute.
