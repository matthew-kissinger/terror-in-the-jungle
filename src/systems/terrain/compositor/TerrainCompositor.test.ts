...
describe('composeTerrain (R2.1 - resolver wired in)', () => {
  describe('Open Frontier (seed 42)', () => {
    const { baseProvider, features } = buildScenario(OPEN_FRONTIER_CONFIG);
    ...
    it('stamp shape (geometry + policy annotations) matches the priority-sorted concat', () => {
      ...
      const legacyShapes = [...features.stamps]
    ...
    it('passes vegetation exclusion zones through', () => {
      const expectedZones = [...features.vegetationExclusionZones];
      expect(composed.vegetationExclusionZones).toEqual(expectedZones);
    });

    it('returns a conflicts list (resolver may resolve overlapping facility/airfield stamps)', () => {
      expect(Array.isArray(composed.conflicts)).toBe(true);
    });
  ...
  describe('A Shau Valley (procedural noise fallback for offline tests)', () => {
    ...
    const { baseProvider, features } = buildScenario(aShauFixtureConfig);
