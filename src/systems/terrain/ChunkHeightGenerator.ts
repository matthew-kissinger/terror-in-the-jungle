...
/**
 * Generates height data for terrain chunks using layered noise
 */
export class ChunkHeightGenerator {
  /**
   * Generate height data array for a chunk
   * @param heightData - Output array to fill (must be pre-allocated)
   * @param chunkX - Chunk X coordinate
   * @param chunkZ - Chunk Z coordinate
   * @param size - Chunk size in world units
   * @param segments - Grid resolution (segments per side)
  ...
  static generateHeightData(
    ...
    const resolution = segments; // Must match geometry segments
