...
export interface GameSettings {
  ...
  touchSensitivity: number;   // 1-10 (UI scale), mapped to 0.004-0.016 internally (Fortnite-level responsiveness)
...
export class SettingsManager {
  ...
  /** Returns touch look sensitivity (0.004 - 0.016). Tuned for Fortnite-level mobile responsiveness. */
  getTouchSensitivityRaw(): number {
    const uiValue = this.settings.touchSensitivity; // 1-10
    return 0.004 + (uiValue - 1) * (0.012 / 9);
  }
