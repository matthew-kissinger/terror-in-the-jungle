...
type ModeHorizonEntry = {
  ...
  farCanopyCoverageDistanceMeters: number;
  effectiveFarCanopyCoverageRadiusMeters: number;
  effectiveVisibleVegetationRadiusMeters: number;
  limitingFactor: 'shader-max-distance' | 'scatterer-residency' | 'terrain-extent' | 'far-canopy-coverage';
...
function analyzeMode(config: GameModeConfig): ModeHorizonEntry {
  ...
  const baseVegetationRadius = Math.min(maxPaletteDistance, residencyCornerRadius, visualHalfExtent);
  const farCanopyCoverageDistance = config.terrain?.farCanopyTint?.enabled === true
    ? Math.max(0, config.terrain.farCanopyTint.coverageDistance ?? 0)
    : 0;
  const farCanopyCoverageRadius = Math.min(farCanopyCoverageDistance, visualHalfExtent);
  const visibleVegetationRadius = Math.max(baseVegetationRadius, farCanopyCoverageRadius);
  ...
  if (maxBareBand >= 300 && farCanopyCoverageRadius <= baseVegetationRadius) {
  ...
  return {
    ...
    farCanopyCoverageDistanceMeters: roundMetric(farCanopyCoverageDistance),
    effectiveFarCanopyCoverageRadiusMeters: roundMetric(farCanopyCoverageRadius),
  ...
  return {
    ...
    assumptions: {
      ...
      notes: [
        ...
        'Explicit farCanopyTint.coverageDistance is counted as the procedural outer-canopy material tier.',
...
function main(): void {
  ...
  const modeSummary = report.modes
    .map((mode) => {
      ...
      return `${mode.id}: visibleVegetation=${mode.effectiveVisibleVegetationRadiusMeters}m, farCanopyCoverage=${mode.effectiveFarCanopyCoverageRadiusMeters}m, maxBareBand=${roundMetric(maxBand)}m, flags=${mode.flags.join(',') || 'none'}`;
    })
    .join('\n');
